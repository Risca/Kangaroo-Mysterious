...
/* Save V4L2_PIX_FMT_YUYV format frame to file */
static int
frame_save(const void *p, const char* filename )
{
    static int packed_value, i;
    struct v4l2_format userfmt;
    FILE* fp = fopen(filename, "w" );

    CLEAR(userfmt);

    userfmt.type                = V4L2_BUF_TYPE_VIDEO_CAPTURE;
    userfmt.fmt.pix.width       = 640;
    userfmt.fmt.pix.height      = 480;
    userfmt.fmt.pix.pixelformat = V4L2_PIX_FMT_YUYV;
    userfmt.fmt.pix.field       = V4L2_FIELD_INTERLACED;
    // Write PNM header
    fprintf( fp, "P6\n" );
...
process_image                   (const void *           p)
{
    ...
    char filename[255];
    sprintf(filename,"frames/savedframe%i.ppm",count);
    frame_save(p,filename);
                                 ...
                                 char **                argv)
{
        ...
        printf("Press any key to start processing\n");
        system("read");
